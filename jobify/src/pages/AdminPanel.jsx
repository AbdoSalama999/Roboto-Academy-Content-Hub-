...
function AdminPanel() {
  ...
  useEffect(() => {
    ...
    axios
      .get('http://localhost:5555/api/tracks', config)
      .then((res) => setTracks(res.data.data || []))
      .catch((err) => console.error('Error fetching tracks:', err))

    axios
      .get('http://localhost:5555/api/courses', config)
  ...
  const fetchDayReports = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5555/api/daily-reports/summary?date=${selectedDate}`,
        getAuthHeader(),
      )
      if (res.data?.success) {
  ...
  const fetchInstructorStats = async (instructorId) => {
    try {
      const res = await axios.get(
        `http://localhost:5555/api/daily-reports/stats/${instructorId}`,
        getAuthHeader(),
      )
      if (res.data?.success) {
  ...
  const handleCreateCourse = async (e) => {
    ...
    try {
      const res = await axios.post(
        'http://localhost:5555/api/courses',
        {
          title: courseTitle,
  ...
  const handleAddSession = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `http://localhost:5555/api/courses/${foundCourse._id}/sessions`,
        {
          title: sessionTitle,
          notes: sessionNotes,
          files: uploadedFiles,
