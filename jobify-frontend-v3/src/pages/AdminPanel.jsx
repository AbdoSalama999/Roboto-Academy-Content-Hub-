...
function AdminPanel() {
  ...
  // --- States خاصة بسجل تحميلات المستخدمين (Download Logs) ---
  ...
  // جلب سجل التحميلات عند اختيار تبويب اللوجز أو تغيير الفلاتر
  useEffect(() => {
    if (activeTab === 'logs') {
      fetchActivityLogs()
    }
  }, [activeTab, logNameFilter, logDateFilter])

  const fetchActivityLogs = async () => {
    setLoadingLogs(true)
    try {
      const params = new URLSearchParams()
      if (logNameFilter) params.append('name', logNameFilter)
      if (logDateFilter) params.append('date', logDateFilter)

      const res = await axios.get(
        `${API_BASE_URL}/activity/summary${
  ...
  return (
    <div className='page-shell admin-panel'>
      ...
      {activeTab === 'logs' && (
        <section className='card admin-section'>
          <h2>📥 Instructor Download Activity</h2>
          <p className='logs-subtitle'>
            See what each instructor downloaded, grouped by day.
          </p>

          <div className='logs-filter-row'>
            <div className='field logs-filter-field'>
            ...
            {(logNameFilter || logDateFilter) && (
              <button
                className='btn btn-coral logs-clear-btn'
                onClick={() => {
                  setLogNameFilter('')
                  setLogDateFilter('')
                }}
          ...
          {loadingLogs ? (
            <p className='muted-text'>Loading download logs...</p>
          ) : activityLogs.length === 0 ? (
            <p className='muted-text'>No download activity recorded yet.</p>
          ) : (
            <div className='logs-list'>
              {activityLogs.map((entry) => (
                <div
                  key={`${entry.userId}_${entry.date}`}
                  className='card log-day-card'
                >
                  ...
                  <ul className='log-downloads-ul'>
                    {entry.downloads.map((d, idx) => (
                      <li key={idx}>
                        📄 <strong>{d.fileName || 'File'}</strong> —{' '}
                        {d.courseTitle} / {d.sessionTitle}{' '}
